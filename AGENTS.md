...
## Current state (2026-08-26 — Sales Workforce substrate hardened, Tasks 34–58)
- **Sales workforce execution layer (`src/server/sales/`) complete and hardened (Tasks 34–57); Task 58 was documentation hygiene (comment drift fix only — no runtime change).**
  - `workforce.ts` — durable task queue: atomic claim (FOR UPDATE SKIP LOCKED on PG; mutex-serialized on SQLite), bounded retries with backoff, stale-task reaper, global concurrency ceiling, worker schedule/eligibility (`scheduler.ts`).
  - Assignment (`contacts.ts`): idempotent `enqueueOutreach` — channel derived from the WORKER row (never the client), eligibility asserted, UNIQUE(prospect, channel) backstop; append-only attempt ledger (`sales_attempts`).
  - Conversations (`conversations.ts`): OPEN → NEEDS_HUMAN → CLOSED state machine; human-gate parking/resume (Task 44); conversation rows bound only for executed attempts (Task 56).
  - Dispatch boundary (`noopChannel.ts`, `providerContract.ts`) — the fail-safe rule: only channels with a registered adapter execute; `phone`/`instagram_dm`/unknown are refused with a structured REJECTED and **never fall through to the noop executor** (Task 48). No fabricated provider ids or success on refusal.
  - Provider contract (`SalesProviderAdapter`): adapters anchor eligibility on the server-loaded `contact.prospectId` (payload prospect ids ignored — Task 56), consume `dispatch.attemptKey = task.id` as the external idempotency key (Task 50), and classify outcomes (CONNECTED/DELIVERED/TIMEOUT/ERROR/REJECTED).
  - Eligibility recheck at dispatch (Task 46) with typed classification: `ProspectIneligibleError` → permanent refusal; transient errors → retryable (Task 52).
  - **Real phone/Instagram/Twilio/Meta providers are intentionally NOT implemented yet.** Residual known limitation: provider-side eligibility TOCTOU (documented in `docs/sales-toctou.md`) — cannot be solved with more DB transactionality; needs provider-side idempotency.

## Previous state (2026-08-19, Phase B — Sales & Delivery Orchestrator MVP core)
- **Orchestration layer** (23 tests in `tests/orchestration.test.ts` at the time; PG parity tests in `tests/pgTransactions.test.ts`). Production smoke verified on BOTH dialects end-to-end: login → prospect → design → HUMAN approve → submit → job COMPLETED (real tenant+agent, ACTIVE, PUBLISHED version) → delivery → acceptance; idempotent resubmit returns the same job; duplicate acceptance rejected; unauth 401 / tenant 403.
- **ORCHESTRATION LAYER (new, `src/server/orchestration/`)**: prospects → design proposals → HUMAN approval → idempotent factory submission → delivery → acceptance. Platform-owner-only routes mounted at `/api/orchestration/*` (`requireAuth` + `requireRole('PLATFORM_OWNER')`). Prospects are platform-owned until conversion (`prospect.businessId` set by the factory submitter when the tenant is created). No LLM anywhere in the layer — LLMs may produce design artifacts later, but transitions are deterministic code only.
  - `tables.ts` — single-source DDL + `initOrchestrationTables` self-heal (wired into `db.init()`). Migrations: `migrations/016_orchestration.sql` (SQLite) + `migrations/pg/017_orchestration.sql` (PG — 017 because pg/016_dialect_parity occupies 016; the PG sequence is independent). Tables: `prospects`, `design_proposals`, `factory_jobs` (UNIQUE idempotency_key), `deliveries`, `acceptances` (UNIQUE delivery_id).
  - `prospects.ts` / `design.ts` / `factoryJobs.ts` / `deliveries.ts` — explicit state-machine maps; illegal transitions throw. Design approval is a HUMAN-only route-local action; `validateDesignConfiguration` gates approval.
  - `factorySubmitter.ts` — the ONLY bridge into the factory. Drives the authoritative factory services in order (create tenant/agent → `runEvaluation` → `runSelfCorrection` if failing → `publishVersion` → `transitionAgentStatus('ACTIVE')` → delivery) — reimplements NOTHING; evaluation/correction/publish/readiness gates stay authoritative (a failing design DEAD_LETTERs at the exact step, never bypasses). Idempotent: same idempotency key returns the existing job; one job per design; UNIQUE index backstop for concurrent submits.
- **`src/server/agentLifecycle.ts` (new, extracted)**: the previously-inline route logic is now the authoritative lifecycle service shared by routes.ts AND the orchestrator — `createBusinessTenant`, `createAgentWithInitialDraft`, `transitionAgentStatus`, `AGENT_STATUS_TRANSITIONS`/`AGENT_STATUSES`, `pauseOtherActiveAgents`. routes.ts delegates; behavior verified identical (full suite green pre/post refactor). The transition map is the RICH original (DRAFT→TESTING/PAUSED/ARCHIVED, TESTING→READY/DRAFT/..., PAUSED resumes `pausedFrom`).
- **Telemetry**: `recordOrchestrationEvent` (telemetry.ts) + 8 new event types (`PROSPECT_CREATED`, `DESIGN_CREATED`, `DESIGN_APPROVED`, `FACTORY_JOB_STARTED`, `FACTORY_JOB_STEP`, `FACTORY_JOB_FAILED`, `AGENT_DELIVERED`, `DELIVERY_ACCEPTED`). Pre-conversion events are scoped to sentinel businessId `'platform'` (telemetry_events.business_id is a scope tag, no FK); post-conversion events use the real tenant id. Same safe-summary discipline (no PII/secrets/args).
- **UI**: `src/components/OrchestrationView.tsx` + "Orchestration" tab in `PlatformOwnerDashboard` (prospects/designs/jobs/deliveries, approve + submit + accept buttons; config entered as JSON).
- **Nullability drift rule re-confirmed**: PG test caught `design_proposals.agent_type NOT NULL` crashing on absent-field explicit-NULL inserts → aligned ALL three DDL sources (tables.ts, 016 sqlite, 017 pg): optional columns nullable.
- Tests: 29 in `tests/orchestration.test.ts` (CRUD, human gate, happy path, gate authority for evaluation/correction/publish/activation, idempotency, duplicate acceptance, auth 401/403, isolation, malformed/malicious input, secret non-echo, SQL-injection safety, FAILED + DEAD_LETTERED terminals) + 2 PG parity tests in `tests/pgTransactions.test.ts`.

## Previous state (2026-08-19, P1 — PostgreSQL production path repaired end-to-end)
