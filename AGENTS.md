...
## Current state (2026-08-19, Phase B — Sales & Delivery Orchestrator MVP core)
- **431 tests across 32 files** (422 passing + 9 PG-skipped without PG_TEST_URL; with PG_TEST_URL all 431 run green). `npx tsc --noEmit` clean, `npm run build` clean. Production smoke verified on BOTH dialects end-to-end: login → prospect → design → HUMAN approve → submit → job COMPLETED (real tenant+agent, ACTIVE, PUBLISHED version) → delivery → acceptance; idempotent resubmit returns the same job; duplicate acceptance rejected; unauth 401 / tenant 403.
