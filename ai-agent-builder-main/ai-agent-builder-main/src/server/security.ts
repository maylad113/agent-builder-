...
export class SqliteRateLimitStore implements RateLimitStore {
  ...
  constructor(sqlite?: BetterSqlite3.Database) {
    ...
    if (!this.sqlite) {
      // Postgres dialect (no better-sqlite3 handle): the SQLite store cannot
      // operate. Throwing at construction would crash module load, so callers
      // must select the memory store for non-sqlite dialects. This guard makes
      // the failure explicit instead of a cryptic `.prepare of undefined`.
      throw new Error('SqliteRateLimitStore requires a better-sqlite3 handle (db.sqlite is undefined — set RATE_LIMIT_STORE=memory for non-sqlite dialects).');
    }
    // Self-provision the table so the store works regardless of migration
    // ordering: selectStore() runs at module-import time (before db.init()),
    // and tests construct the store directly over a scratch DB. Idempotent.
    this.sqlite.exec(
