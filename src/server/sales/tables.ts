...
export async function initSalesWorkforceTables(client: DbClient): Promise<void> {
  ...
  await client.exec(`CREATE TABLE IF NOT EXISTS sales_conversations (
    id                      TEXT PRIMARY KEY,
  ...
  await client.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_sales_conversations_provider ON sales_conversations(provider_conversation_id)');
  await client.exec('CREATE INDEX IF NOT EXISTS idx_sales_conversations_contact ON sales_conversations(contact_id)');
  await client.exec('CREATE INDEX IF NOT EXISTS idx_sales_conversations_status ON sales_conversations(status)');

  await client.exec(`CREATE TABLE IF NOT EXISTS sales_conversation_turns (
    id              TEXT PRIMARY KEY,
  ...
  await client.exec('CREATE INDEX IF NOT EXISTS idx_sales_turns_conversation ON sales_conversation_turns(conversation_id, created_at)');
}
